...
timer2::timer2 (uint8_t wgm, uint8_t prsclr = 1, uint8_t depth = 0) 
{
	tccrb = prsclr;
	ocra = depth;
	setMode(wgm);
...
void timer2:: config()
{
	TCCR2A = tccra & 0xF3; // remove pin states
	if (tccra & 0x08) DDRD |= 1<<6;
	if (tccra & 0x04) DDRD |= 1<<5;
	TCCR2B = tccrb;
	TIMSK2 = timsk;
...
void timer2:: setMode(uint8_t wgm)
{
	tccrb &= ~0x08;
	tccrb |= (wgm&4)<<1;
	tccra &= ~0x03;
...
void timer2::setChannelA(uint8_t com, uint8_t pinmode = 1)
{
	tccra &= ~0xC0;
	tccra |= com<<6;
	if (pinmode) tccra |= 1<<3;
...
void timer2::setChannelB(uint8_t com, uint8_t pinmode = 1)
{
	tccra &= ~0x30;
	tccra |= com<<4;
	if (pinmode) tccra |= 1<<2;
