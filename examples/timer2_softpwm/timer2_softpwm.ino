...
timer2 test(PWM_FAST, CLOCK_PS256);
digitalPin led(&PORTB, 5, OUTPUT);
usart term; // used for demo code in loop()
uint8_t blink_speed = 5; //0 off, 1 always on (extremely fast), 2..7 more is slower
...
{
  led.low(); // turn the led off at compare match
}
...
term << "Change speed: " >> blink_speed << "\r\nSpeed set to " << blink_speed%8 << "\r\n";
test.setPrescaler(blink_speed%8);
test.enable();
}
